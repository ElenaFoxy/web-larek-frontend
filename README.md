...
##### Свойства:
`private _basket: string[]` - товары в корзине. 
...
`private _order: IOrder = ` - заказ.  
 ...
 ##### Методы:   
`get basket(): string[]` - получить список товаров в корзине для basket.   
`set basket(value: string[])` - установить список товаров в корзине для basket.   
...
`constructor(container: HTMLElement, protected events: IEvents)` - инициализирует элементы страницы, настраивает обработчик события для клика по корзине, который вызывает событие basket:open.    
##### Свойства:
...
`toggleCard(state: boolean = true)` - управляет кнопкой способа оплаты онлайн.    
`toggleCash(state: boolean = true)` - управляет кнопкой способа оплаты при получении.    
...
`toggleButton()` - переключение кнопки в активное/неактивное состояние 
